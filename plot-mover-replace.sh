...
# Set the maximum size of the files to be replaced in GB
max_size=100
...
function start_transfer() {
    ...
    # Delete .plot files on the destination drives that are larger than max_size to make room for the new plot
    dest_files=$(ssh -T -c aes128-ctr -o Compression=no -x $user@$destination "find ${farming_drives[*]} -name '*.plot' -size +${max_size}G -print0")
    total_size=0
    while read -d '' dest_file; do
        echo "🔥 Deleting $dest_file on $drive"
        ssh -T -c aes128-ctr -o Compression=no -x $user@$destination "rm -f $dest_file"
        size=$(ssh -T -c aes128-ctr -o Compression=no -x $user@$destination "du -b $dest_file | cut -f1")
        total_size=$((total_size + size))
        if [ "$total_size" -ge "$((max_size * 1000000000))" ]; then
            break
        fi
    done < <(echo -n "$dest_files")

    # Start the transfer using rsync in a detached screen session
    echo "🚜 Transferring $file to $drive"
    rsync -avz --remove-source-files -e "ssh -T -c aes128-ctr -o Compression=no -x" $file $user@$destination:$drive &
    pids+=($!)
    moved_files+=("$file")
    last_drive="$drive"
